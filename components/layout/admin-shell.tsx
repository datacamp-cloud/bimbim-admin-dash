...
const nav = [
  { section: '', items: [{ label: 'Tableau de bord', href: '/dashboard', icon: LayoutDashboard }] },
  ...
  { section: 'COMMANDES & LIVRAISONS', items: [
    { label: 'Toutes les commandes', href: '/deliveries?view=orders', icon: Box },
    { label: 'Toutes les livraisons', href: '/deliveries', icon: Box },
    { label: 'Suivi en temps réel', href: '/deliveries?view=live', icon: MapPin },
    { label: 'Retour de colis', href: '/deliveries?view=returns', icon: Package, count: 3 },
  ]},
  { section: 'BIMBIM PAY', items: [
    { label: 'Transactions', href: '/settings?tab=transactions', icon: Wallet },
    { label: 'Retraits / Dépôts', href: '/settings?tab=wallet', icon: Wallet },
    { label: 'Factures', href: '/settings?tab=invoices', icon: FileText },
  ]},
  { section: 'CONTENU & COMMUNICATION', items: [
    { label: 'Notifications', href: '/notifications', icon: Bell, count: 3 },
    { label: 'Messages', href: '/notifications?view=messages', icon: MessageSquare },
    { label: 'Actualités', href: '/notifications?view=news', icon: Newspaper },
  ]},
...
export function AdminShell({ children, title, subtitle, action }: { children: React.ReactNode; title: string; subtitle?: string; action?: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className={cn('fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-sidebar-border bg-sidebar px-4 py-5 text-sidebar-foreground transition-transform lg:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
    ...
    <div className="lg:pl-64">
      ...
      <main className="mx-auto max-w-[1480px] px-5 py-7 md:px-8 md:py-8">
        {title && <div className="mb-6"><p className="mb-1 text-sm text-muted-foreground" suppressHydrationWarning>{today}</p><h1 className="text-2xl font-semibold tracking-tight">{title}</h1>{subtitle && <p className="mt-1 text-sm text-muted-foreground">{subtitle}</p>}</div>}
        {children}
      </main>
    </div>
  </div>
