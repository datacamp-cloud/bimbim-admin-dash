        ...
        <Card className="overflow-hidden">
          <div className="flex flex-col gap-4 border-b border-border p-5">
            ...
            <div className="flex flex-wrap items-center gap-2" role="group" aria-label="Filtrer par statut">
              {statusOptions.map((option) => <button key={option} onClick={() => setStatus(option)} className={cn('rounded-lg px-3 py-2 text-xs font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-dark', status === option ? 'bg-primary text-primary-foreground shadow-sm hover:bg-primary' : 'bg-muted text-muted-foreground hover:bg-border hover:text-foreground')}>{option}</button>)}
