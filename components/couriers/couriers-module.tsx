        ...
        <Card className="overflow-hidden">
          <div className="flex flex-col gap-4 border-b border-border p-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              ...
              <select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)} aria-label="Trier les coursiers" className="rounded-xl border border-border bg-card px-3 py-2.5 text-sm outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50"><option value="activity">Dernière activité</option><option value="deliveries">Plus de livraisons</option><option value="rating">Meilleure note</option></select>
            </div>
            <div className="flex flex-wrap items-center gap-2" role="group" aria-label="Filtrer par statut">
              {statusOptions.map((option) => <button key={option} onClick={() => setStatus(option)} className={cn('rounded-lg px-3 py-2 text-xs font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50', status === option ? 'bg-primary text-primary-foreground shadow-sm hover:bg-primary' : 'bg-muted text-muted-foreground hover:bg-border hover:text-foreground')}>{option}</button>)}
              <span className="hidden h-5 w-px bg-border sm:block" />
              <div className="flex items-center gap-2"><MapPin className="size-3.5 text-muted-foreground" /><select value={zone} onChange={(event) => setZone(event.target.value)} aria-label="Filtrer par zone" className="bg-transparent text-xs font-medium text-muted-foreground outline-none transition-colors focus:text-foreground focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50"><option value="Toutes">Toutes les zones</option>{zoneOptions.slice(1).map((option) => <option key={option} value={option}>{option}</option>)}</select><ChevronDown className="pointer-events-none -ml-5 size-3.5 text-muted-foreground" /></div>
            </div>
          </div>
