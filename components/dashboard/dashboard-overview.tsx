...
export const fallback: DashboardData = {
  counters:{clients:86,partners:86,livreurs:342,commandes:1248,livraisons:1153,pendingReturns:3,onlineLivreurs:342},
...
function SectionTitle({title,link}:{title:string;link?:string}){return <div className="mb-4 flex items-center justify-between"><h2 className="text-[15px] font-semibold">{title}</h2>{link&&<Link href={link} className="rounded-md text-xs font-semibold text-primary transition-colors hover:text-primary-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">Voir toutes →</Link>}</div>}

export function DashboardOverview({data}:{data?:DashboardData}) {
  const dataSource = data ?? fallback
  const [period,setPeriod]=useState('Ce mois')
  const [refreshing,setRefreshing]=useState(false)
  const delta=dataSource.revenue.previous?Math.round(((dataSource.revenue.current-dataSource.revenue.previous)/dataSource.revenue.previous)*100):22
  ...
  return <AdminShell title="" subtitle="">
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {[
          ['Total commandes',dataSource.counters.commandes.toLocaleString('fr-FR'),'+12%',Package,'text-primary bg-primary/10'],
          ['Livraisons effectuées',dataSource.counters.livraisons.toLocaleString('fr-FR'),'+15%',CheckCircle2,'text-success bg-success/10'],
          ['Partenaires actifs',dataSource.counters.partners.toLocaleString('fr-FR'),'+8%',UserRound,'text-info bg-info/10'],
          ['Coursiers actifs',dataSource.counters.onlineLivreurs.toLocaleString('fr-FR'),'+11%',Truck,'text-warning bg-warning/10'],
          ['Bimbim Pay · Solde global',money(dataSource.revenue.current),'+'+delta+'%',CircleDollarSign,'text-primary bg-primary/10'],
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Panel className="p-5"><SectionTitle title="Bimbim Pay" link="/settings?tab=wallet"/><p className="text-2xl font-bold">{money(dataSource.revenue.current)}</p><p className="text-xs text-muted-foreground">Solde disponible</p><div className="mt-4 grid grid-cols-2 gap-2"><button className="flex items-center justify-center gap-2 rounded-xl bg-primary px-3 py-2.5 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary-dark"><ArrowDownToLine className="size-3.5"/>Dépôt</button><button className="flex items-center justify-center gap-2 rounded-xl border border-border px-3 py-2.5 text-xs font-semibold transition-colors hover:bg-muted hover:text-foreground"><ArrowUpRight className="size-3.5"/>Retrait</button></div><div className="mt-4 border-t border-border pt-3">{dataSource.recentTransactions.slice(0,3).map(t=><div key={t.id} className="flex justify-between border-b border-border py-2 last:border-0"><span className="text-[10px]">{t.type_transaction==='credit'?'Dépôt par partenaire':t.type_transaction==='commission'?'Frais de service':'Retrait vers compte'}<small className="block text-muted-foreground">{dateLabel(t.date_operation)}</small></span><b className={cn('text-[10px]',t.montant>=0?'text-success':'text-danger')}>{t.montant>=0?'+':''}{money(t.montant)}</b></div>)}</div></Panel>
        ...
        <Panel className="p-5"><SectionTitle title="Retour de colis" link="/deliveries?view=returns"/>{dataSource.returns.slice(0,3).map(r=><div key={r.id} className="flex items-center gap-3 border-b border-border py-3 last:border-0"><Package className="size-4 text-danger"/><div className="min-w-0 flex-1"><b className="font-mono text-[10px]">{r.id}</b><p className="truncate text-[10px] text-muted-foreground">{dateLabel(r.date)} · {r.courier}</p></div><span className={cn('text-[10px] font-semibold',r.statut==='en_cours'?'text-warning':r.statut==='livre'?'text-success':'text-muted-foreground')}>{statusLabel(r.statut)}</span></div>)}</Panel>
      </div>

      <Panel className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><span className="flex size-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><Package className="size-4"/></span><div><p className="text-sm font-semibold">Colis retour</p><p className="text-xs text-muted-foreground">Gérez les retours de colis depuis la livraison jusqu'à la réception.</p></div></div><Link href="/deliveries?view=returns" className="rounded-xl bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary-dark">Accéder au suivi →</Link></Panel>
      <div className="flex justify-end"><button onClick={refresh} className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-[11px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50">{refreshing?<RefreshCw className="size-3.5 animate-spin"/>:<RefreshCw className="size-3.5"/>}Actualiser</button></div>
    </div>
  </AdminShell>
}
